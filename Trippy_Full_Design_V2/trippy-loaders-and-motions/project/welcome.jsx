/* ════════════════════════════════════════════════════════════════
 *  Trippy — Destination welcome animation (wordless)
 *  Plays when a trip opens. The destination's flag colors wash in as
 *  waving ribbons, the compass orbits assemble over them, then converge
 *  into the Trippy mark — recolored to that destination. No words:
 *  color alone says where you're going.
 *  Props: { theme, speed, dark, duration, loop, onDone }
 * ════════════════════════════════════════════════════════════════ */
function WelcomeStyles() {
  return (
    <style>{`
      ...
      @keyframes wlHalo   { 0%,100% { transform: scale(.9); opacity:.6; } 50% { transform: scale(1.08); opacity:1; } }
      @keyframes wlPing   { 0% { transform: scale(.4); opacity:.8; } 100% { transform: scale(2.4); opacity:0; } }
    `}</style>
  );
}

function WelcomeAnimation({ theme, speed = 1, dark = false, duration = 4, loop = false, onDone }) {
  const t = theme;
  ...
  React.useEffect(() => {
    ...
    const tick = (now) => {
      ...
      let prog = (now - start.current) / (dur * 1000);
      if (prog >= 1) {
        if (loop) { start.current = now; prog = 0; }
        else { prog = 1; }
      }
      setP(prog);
      if (prog < 1 || loop) raf.current = requestAnimationFrame(tick);
  ...
  const back = (x) => { const c = 1.7; const u = x - 1; return 1 + (c + 1) * u * u * u + c * u * u; };
  ...
  const sp = (s) => (s / speed) + 's';

  const Orbit = (r, dash, color, w, dir, durS, op) => (
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden',
      ...
      <WelcomeStyles />
      {/* flag-color ribbons, waving */}
      <div style={{ position: 'absolute', inset: '-30%', opacity: ribbonOpacity,
      ...
      {/* paper veil so colors read as a soft wash, not a flat flag */}
      ...
      {/* resolve: the recolored Trippy compass mark */}
      <div style={{ position: 'absolute', width: '46%', aspectRatio: '1',
        transform: `scale(${markScale})`, opacity: markOpacity }}>
        {/* ping ring at the resolve */}
        {markP > 0 && markP < 1 && (
          <div style={{ position: 'absolute', inset: '14%', borderRadius: '50%',
            border: `2px solid ${t.c2}`, animation: `wlPing ${sp(1.1)} ease-out infinite` }} />
        )}
...
Object.assign(window, { WelcomeAnimation, WelcomeStyles });
